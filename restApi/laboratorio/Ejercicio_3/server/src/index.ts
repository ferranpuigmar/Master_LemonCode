...
import { respondWithGraphiQL } from '@hono/graphql-server';
...
const graphqlHandler = createHandler({ schema: createGraphqlServer() });

app.get('/graphql', (context) =>
  context.req.header('accept')?.includes('text/html')
    ? respondWithGraphiQL(context)
    : graphqlHandler(context.req.raw)
);

app.post('/graphql', (context) => graphqlHandler(context.req.raw));
